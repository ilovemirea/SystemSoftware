...
public class Compiler {
    ...
    public static void listOut(List<String> lines) {
    ...
    public static void peepholeOptimize(){
        Pattern p = Pattern.compile("\\s*PUSH (.+)");
        Pattern p1 = Pattern.compile("\\s*POP (.+)");
        for(int i = 0; i < asm.size()-1; i++){
            String a = asm.get(i);
            String b = asm.get(i + 1);
            Matcher m = p.matcher(a);
            Matcher m1 = p1.matcher(b);
            if(m.matches() && m1.matches()){
                String arg1 = m.group(1);
                String arg2 = m1.group(1);
    ...
    public static ExprNode foldConstants(ExprNode node){
        if (node instanceof NumberNode) return node;
        else if (node instanceof BinOpNode) {
            BinOpNode binOp = (BinOpNode) node;
            ExprNode l = foldConstants(binOp.left);
            ExprNode r = foldConstants(binOp.right);
            ...
            }else return new BinOpNode(binOp.op, l, r);
        }else if (node instanceof VarNode) return node;
